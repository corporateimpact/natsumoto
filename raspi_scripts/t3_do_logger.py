#!/usr/bin/python
# -*- coding: utf-8 -*-
# 基本的に、サンプルスクリプトの細かいところを改造して利用しています
...
class AtlasI2C:
    ...
    def read(self, num_of_bytes=31):
        ...
        if response[0] == 1:             # if the response isn't an error
            ...
            char_list = map(lambda x: chr(x & ~0x80), list(response[1:]))            #print(char_list)
            # NOTE: having to change the MSB to 0 is a glitch in the raspberry pi, and you shouldn't have to do this!
            item = str("" + ''.join(char_list))
            re_list.append(item.strip('\x00'))
            # ↑改造箇所 そのままだと大量の\x00がまぎれたままになるので削除する処理を追加
            #return "" + ''.join(char_list) #''.join(char_list)     # convert the char list to a string and returns it
            # ↑改造箇所 結果メッセージが邪魔だったので最小限に でも最初の処理がprintから始まるのでどうしてもNoneが消えない
        else:
...
def main():
    ...
    try:
        for i in range(3):
            ...
            i += 1
            # ↑改造箇所 サンプルではコマンドで停止するまで無限ループするタイプだったので3回に制限
            # 取得地は平均を使用(処理はgetdata.pyにて)
